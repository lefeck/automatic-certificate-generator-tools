...
usage() {
  cat << EOF
Usage: $(basename "${BASH_SOURCE[0]}") [-a [rsa|ecc]] [-d <domain>] [-n <name>] [-t <days>] [-p <password>] [-s <show>] [-h] [-v]"
...
-s --show              Whether to display the service certificate password in the foreground, the default is false.
...
random_str=$(cat /proc/sys/kernel/random/uuid |md5sum|cut -c 1-8 )
...
function parse_param() {
    ...
    getopt_cmd=$(getopt -o a:d:n:t:p:shv --long algorithm:,domain:,name:,password:,days:,show,help,verbose -n $(basename $0) -- "$@")
    if [ $? -ne 0 ]; then
        exit 1
    fi
    eval set -- "$getopt_cmd"
...
function generate_server_key_and_crt_file() {
    ...
    if [ ! -f "${srv_key_file}" ]; then
        ...
        if [ ${show} == "true" ]; then
          log "👍 certificate server password is ${password}"
        fi

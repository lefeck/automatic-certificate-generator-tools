...
usage() {
  cat << EOF
Usage: $(basename "${BASH_SOURCE[0]}") [-a [rsa|ecc]] [-d <domain>] [-n <name>] [-t <days>] [-h] [-v]"
...
function parse_param() {
    ...
    getopt_cmd=$(getopt -o a:d:n:t:hv --long algorithm:,domain:,name:days,help,verbose -n $(basename $0) -- "$@")
    if [ $? -ne 0 ]; then
        exit 1
    fi
    eval set -- "$getopt_cmd"
...
function generate_server_key_and_crt_file() {
    ...
    if [ ! -f "${srv_key_file}" ]; then
        ...
        # Convert CRT to PFX
        openssl pkcs12 -inkey ${srv_key_file} -in {srv_crt_file} -export -out ${srv_pfx_file}
